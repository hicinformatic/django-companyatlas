#!/usr/bin/env python3
"""Django CompanyAtlas development tool for building, testing, and managing."""
...
PYTHON_COMPANYATLAS_DIR = PROJECT_ROOT.parent / 'python-companyatlas'
...
def task_help():
    ...
    print(f"{BLUE}django-companyatlas — available commands{NC}\n")
    ...
    print("  update-companyatlas     Install or refresh local python-companyatlas")
...
def task_update_companyatlas():
    """Install or update python-companyatlas from local directory."""
    if not venv_exists():
        print_error("Virtual environment not found. Run 'python dev.py venv' first.")
        return False

    args = sys.argv[2:]
    target_dir = Path(args[0]) if args else PYTHON_COMPANYATLAS_DIR

    if not target_dir.exists():
        print_error(
            f"python-companyatlas directory not found at {target_dir}. "
            "Provide path: python dev.py update-companyatlas /path/to/python-companyatlas"
        )
        return False

    print_info("Installing python-companyatlas into the virtual environment...")
    if run_command([str(PIP), "install", "-e", str(target_dir)]):
        print_success("python-companyatlas installed/updated successfully.")
        return True

    print_error("Failed to install/update python-companyatlas.")
    return False
...
def task_coverage():
    ...
    pytest = VENV_BIN / ("pytest.exe" if platform.system() == "Windows" else "pytest")
    if run_command([str(pytest), "--cov=companyatlas", "--cov-report=html", "--cov-report=term"]):
...
def task_lint():
    ...
    success = True
    if not run_command([str(ruff), "check", "companyatlas", "tests"]):
        success = False
    if not run_command([str(mypy), "companyatlas"]):
        success = False
...
def task_format():
    ...
    ruff = VENV_BIN / ("ruff.exe" if platform.system() == "Windows" else "ruff")
    if run_command([str(ruff), "format", "companyatlas", "tests"]):
...
COMMANDS = {
    ...
    "update-companyatlas": task_update_companyatlas,
